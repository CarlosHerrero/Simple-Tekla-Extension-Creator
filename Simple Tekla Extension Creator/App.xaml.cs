    ...
    public partial class App : Application
    {
        protected override void OnStartup(StartupEventArgs e)
        {
            ...
            if (options.ShowHelp)
            {
                string help = "Usage:\n"
                    ...
                    + "SimpleTeklaExtensionCreator.exe --non-interactive --project <Name> [--version <...>] [--ui <...>]";
            ...
            if (options.NonInteractive)
            {
                ...
                MainWindow = new MainWindow(version, ui);
                bool created = MainWindow is MainWindow window
                    && window.TryCreateProjectNonInteractive(options.ProjectName, ui, version, out result);
        ...
        private static CliOptions ParseArguments(string[] args)
        {
            ...
            for (int i = 0; i < args.Length; i++)
            {
                ...
                {
                    return new CliOptions(version, ui, projectName, nonInteractive, true, null);
                }

                if (!arg.StartsWith("--", StringComparison.Ordinal))
                {
                    return new CliOptions(version, ui, projectName, nonInteractive, false, $"Unexpected argument '{arg}'.");
                }
                ...
                else
                {
                    ...
                    if (requiresValue)
                    {
                        ...
                        {
                            return new CliOptions(version, ui, projectName, nonInteractive, false, $"Missing value for '--{key}'.");
                        }
                ...
                if (key.Equals("version", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    {
                        return new CliOptions(version, ui, projectName, nonInteractive, false, "Missing value for '--version'.");
                    }
                ...
                if (key.Equals("ui", StringComparison.OrdinalIgnoreCase))
                {
                    if (string.IsNullOrWhiteSpace(value))
                    {
                        return new CliOptions(version, ui, projectName, nonInteractive, false, "Missing value for '--ui'.");
                    }
                    ...
                    || key.Equals("name", StringComparison.OrdinalIgnoreCase))
                {
                    if (string.IsNullOrWhiteSpace(value))
                    {
                        return new CliOptions(version, ui, projectName, nonInteractive, false, "Missing value for '--project'.");
                    }
                    ...
                    || key.Equals("headless", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    return new CliOptions(version, ui, projectName, nonInteractive, false, $"Invalid boolean value '{value}' for '--{key}'.");
                }

                return new CliOptions(version, ui, projectName, nonInteractive, false, $"Unknown option '--{key}'.");
            }

            return new CliOptions(version, ui, projectName, nonInteractive, false, null);
        }
